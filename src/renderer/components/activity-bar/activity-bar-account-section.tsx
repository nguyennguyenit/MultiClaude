  ...
  if (collapsed) {
    return (
      <div className="relative group px-1">
        <div className="flex justify-center px-3 pt-1.5 pb-0.5 cursor-pointer hover:bg-[var(--mc-bg-hover)] rounded transition-colors">
          {avatarUrl ? (
        ...
        <div className="absolute bottom-full left-full mb-1 ml-1 px-3 py-2 bg-[var(--mc-bg-tertiary)] text-xs rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-50 pointer-events-none transition-opacity shadow-lg border border-[var(--mc-border)]">
          <div className="flex items-center gap-2 mb-1">
            {avatarUrl ? (
              ...
              <div className="w-5 h-5 flex-shrink-0 bg-[var(--mc-bg-tertiary)] border border-[var(--mc-border)] flex items-center justify-center text-[10px]" style={{ borderRadius: '50%' }}>
                {username.slice(0, 1).toUpperCase()}
              </div>
            )}
            <span className="font-medium">{isAuthenticated ? username : 'Not signed in'}</span>
            {isAuthenticated && <span className="w-2 h-2 bg-green-400 flex-shrink-0" style={{ borderRadius: '50%' }} />}
          </div>
          {gitStatus?.branch && (
          ...
          {gitConfig.userEmail && (
            <div className="flex items-center gap-1.5 text-[var(--mc-text-muted)] mt-0.5">
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <span>{gitConfig.userEmail}</span>
            </div>
          )}
        </div>
      </div>
