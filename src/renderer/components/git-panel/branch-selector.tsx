  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        disabled={isLoading}
        className="flex items-center gap-1 px-2 py-0.5 text-xs bg-[var(--mc-bg-hover)] hover:bg-[var(--mc-bg-active)] rounded"
      >
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
            d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
        </svg>
        <span className="text-green-400 max-w-[140px] truncate">{currentBranch || 'N/A'}</span>
        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="max-h-40 overflow-y-auto">
            {localBranches.map(branch => (
              <button
                ...
                className={`w-full px-3 py-1.5 text-left text-xs flex items-center gap-2 hover:bg-[var(--mc-bg-hover)] ${branch.current ? 'text-green-400' : ''
                  }`}
              >
                {branch.current && (
